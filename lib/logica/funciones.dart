...
class Funciones {
  ...
  static void btn1ActualizarReporte(BuildContext context) {
    //Botón Tipo 1
    ...
    correcto.then(
      (futuro) {
        if (futuro == 0) {
          if (ValoresActivos.usuario.tipo as int == 1) {
            Navigator.pushNamed(context, PantallaHome1.ruta);
          } else if (ValoresActivos.usuario.tipo as int == 2) {
            Navigator.pushNamed(context, PantallaHome2.ruta);
          } else if (ValoresActivos.usuario.tipo as int == 3) {
            Navigator.pushNamed(context, PantallaHome3.ruta);
          } else if (ValoresActivos.usuario.tipo as int == 4) {
            Navigator.pushNamed(context, PantallaHome4.ruta);
          }
        } else {
          showModalBottomSheet(
  ...
  static void btn1ActualizarCategoria(BuildContext context) {
    AprobarCategoria aprobarCategoria = AprobarCategoria(Categoria(
      nombre: Argumentos.argsNuevaCategoria[0].text,
      descripcion: Argumentos.argsNuevaCategoria[1].text,
      espacio: Argumentos.argsCategoriaActual.espacio,
    ));

    aprobarCategoria.aprobar().then((futuro) {
      AprobarCategoria eliminarPendiente =
          AprobarCategoria(Argumentos.argsCategoriaActual);
      eliminarPendiente.eliminarPendiente().then((futuro) {
  ...
  static void btn4EditarReporte(BuildContext context, Reporte reporte) {
    //Botón Tipo 4
    Argumentos.argsReporteActual = reporte;
    Navigator.pushNamed(context, PantallaEditarReporte.ruta);
  }
  ...
  static void btn4AprobarCategoria(BuildContext context, Categoria categoria) {
    AprobarCategoria aprobarCategoria = AprobarCategoria(categoria);
    aprobarCategoria.aprobar().then((futuro) {
  ...
  static void btn4EditarCategoria(BuildContext context, Categoria categoria) {
    Argumentos.argsCategoriaActual = categoria;
    Navigator.pushNamed(context, PantallaEditarCategoria.ruta);
  }

  static void btn4RechazarCategoria(BuildContext context, Categoria categoria) {
    AprobarCategoria aprobarCategoria = AprobarCategoria(categoria);
    aprobarCategoria.eliminarPendiente().then((futuro) {
      PantallaCategoriasPendientes.listViewTipo3.actualizar!();
    });
